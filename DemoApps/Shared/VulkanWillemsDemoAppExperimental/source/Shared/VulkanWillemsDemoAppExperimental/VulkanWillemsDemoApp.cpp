...
namespace Fsl
{
  ...
  namespace Willems
  {
    ...
    void VulkanWillemsDemoApp::SetupRenderPass(const VkFormat colorFormat, const VkFormat depthFormat)
    {
      ...
      // The depth attachment is shared by all frames, so the depth clear (and its layout transition) must wait for the previous frames depth
      // writes
      dependencies[0].srcSubpass = VK_SUBPASS_EXTERNAL;
      dependencies[0].dstSubpass = 0;
      dependencies[0].srcStageMask =
        VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
