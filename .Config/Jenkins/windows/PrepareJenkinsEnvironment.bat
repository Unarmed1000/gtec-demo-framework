...
echo FSL_MSVC_ENV_SETUP=%FSL_MSVC2022_ENV_SETUP%
call "%FSL_MSVC2022_ENV_SETUP%/vcvarsall.bat" amd64
if %errorlevel% neq 0 (
  exit /b %errorlevel%
)
rem Since the VS cmake version doesn't support VS2022 we need to ensure that the system one is found first
set path=%FSL_CMAKE_PATH%;%path%
...
set FSL_CI_FEATURES=[ConsoleHost,WindowHost,Test_RequireUserInputToExit,EarlyAccess,EGL,G2D,OpenCL1.2,GoogleUnitTest,OpenCV,OpenCV5,OpenGLES2,OpenGLES3,OpenGLES3.1,OpenVG,OpenVX,OpenVX1.2,Vulkan,Lib_NlohmannJson,Lib_pugixml]
)
