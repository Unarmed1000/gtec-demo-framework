...
class Writer:
    def __init__(self, output: TextIO, width: int = 78) -> None:
        self.output = output
        self.width = width
    ...
    def variable(self, key: str, value: None | str | int | list[str], indent: int = 0) -> None:
        if value is None:
            return
    ...
    def build(
        ...
        if variables:
            iterator = iter(variables.items()) if isinstance(variables, dict) else iter(variables)
    ...
    def default(self, paths: list[str]) -> None:
...
def as_list(inputObj: str | list[str] | None) -> list[str]:
    if inputObj is None:
        return []
...
def expand(string: str, vars: Any, local_vars: Any = None) -> str:
